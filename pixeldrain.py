...
async def get_video_from_pixeldrain_player(session: aiohttp.ClientSession, player_url: str, is_vip: bool = False):
    ...
    try:
        ...
        if not match:
            logging.warning(f"[PixelDrain] Invalid URL format for {player_url}")
        ...
        if mtype == 'l':
            ...
            if not video_files:
                logging.warning(f"[PixelDrain] No video files found in list {mid}")
                return None, None, None

            largest_video = max(video_files, key=lambda x: x.get('size', 0))
            file_id = largest_video.get('id')
            if not file_id:
                logging.warning("[PixelDrain] Could not determine file ID from the largest video")
                return None, None, None
        ...
        logging.warning(f"[PixelDrain] HTTP error: {http_err}")
        return None, None, None
    except Exception as e:
        logging.warning(f"[PixelDrain] Unexpected error: {e}")
        return None, None, None
