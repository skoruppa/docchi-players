...
async def get_video_from_streamup_player(session: aiohttp.ClientSession, player_url: str, is_vip: bool = False):
    try:
        ...
        if not stream_url:
            ...
            if encrypted_data_match and session_id_match:
                ...
                if not response or not response.strip():
                    logging.warning(f"[StreamUP] Empty response from {s_url}")
                    return None, None, None
                try:
                    stream_info = json.loads(response)
                except json.JSONDecodeError:
                    logging.warning(f"[StreamUP] Invalid JSON from {s_url}: {response[:100]}")
                    return None, None, None

            stream_url = stream_info.get("streaming_url")

        if not stream_url:
            logging.warning("[StreamUP] 'streaming_url' not found.")
            return None, None, None
        ...
        logging.warning(f"[StreamUP] Timeout connecting to {player_url}")
        return None, None, None
    except aiohttp.ClientError as e:
        logging.warning(f"[StreamUP] {type(e).__name__}: {e}")
        return None, None, None
